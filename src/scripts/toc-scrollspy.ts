...
// Manages data-state attribute on .toc-link elements: "unread" | "active" | "read"
...
function getTocLinkForId(id: string): HTMLElement | null {
  return document.querySelector<HTMLElement>(`.toc-link[href="#${id}"]`);
}
...
function init() {
  tocLinks = document.querySelectorAll<HTMLElement>('.toc-link');
